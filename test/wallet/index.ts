...
describe("Converter addresses EVM <=> IMV", () => {
  ...
  const incorrect = ["emv1jyc8jnxjeynktsvvpx8uvkpa5hmup0lym2taxg", "09130794Cd2C92765c18C098FC6583dA5f7c0bfE4"]
  incorrect.forEach(address => {
    it("should convert only support address", () => {
      try{
        convertAddress(address)
        throw new Error("not pass")
      } catch (e) {
        expect(e instanceof Error).to.be.true
        expect(e.message).to.be.eq("unsupported type of address")
      }
    })
  })
})
